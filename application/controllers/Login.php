...
class Login extends CI_Controller {
	...
	public function CheckLogin() {
		...
		if ( !empty( $userinfo ) ) {
			if ( intval( $userinfo['admin_status'] ) == 1 ) {
				...
				// 登陆成功跳转
				EchoJson( TRUE, 1, '登陆成功', $this->config->item( 'domain' ).'/index' );
			} else {
				EchoJson( FALSE, 1000, '账号已被封，请联系超级管理员' );
			}
		} else {
			// 登录失败跳转
			EchoJson( FALSE, 10006, '用户名或密码错误' );
		}
	}
	...
	public function ModifyPassword() {
		...
		if ( $this->lmodel->ModifyAdminInfo( $userinfo['admin_id'], $moddata ) ) EchoJson( TRUE, 1, '操作成功' );
		EchoJson( FALSE, 10005, '操作失败' );
	}
