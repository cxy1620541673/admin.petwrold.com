...
class Authority extends PW_Controller {
	...
	public function AjaxEditAuth() {
		...
		// 更新
		if ( $this->amodel->UpdateAuth( $auth_id, $data ) ) EchoJson( TRUE, 1, '操作成功' );
		EchoJson( FALSE, 10010, '操作失败' );
	}
	...
	public function AjaxDelAuth() {
		...
		// 删除
		if ( $this->amodel->DelAuth( $auth_id ) ) EchoJson( TRUE, 1, '操作成功' );
		EchoJson( FALSE, 10010, '操作失败' );
	}
	...
	public function AjaxChangeLevelRelation() {
		...
		if ( !isset( $level[$this->_admin['al_id']] ) ) {
			if ( $this->_admin['al_id'] == 1 ) {
				EchoJson( FALSE, 10012, '超级管理员等级权限不能删除' );
			} else {
				EchoJson( FALSE, 10012, '没有权限' );
			}
		}
	...
	public function AjaxDelLevel() {
		...
		if ( $al_id == 1 ) EchoJson( FALSE, 1000, '不能删除超级管理员等级' );
		// 删除
		if ( $this->amodel->DelLevel( $al_id ) ) EchoJson( TRUE, 1 , '操作成功' );
		EchoJson( FALSE, 1000, '操作失败' );
	}
	...
	public function AjaxAddLevel() {
		...
		// MustInt( $al_level );
		// MustStr( $al_name );
		if ( $al_level < 0 ) EchoJson( FALSE, 1000, '参数错误' );
		// 只有超级管理员可以添加
		// if ( $this->_admin['al_id'] != 1 ) EchoJson( FALSE, 1000, '只有超级管理员可以添加' );
		// 添加
